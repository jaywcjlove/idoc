...
function build(commander){
    ...
    file.readMDSync(todir + 'md/').replace(/\,$/,'').split(',').forEach(function(item,idx){

        var navHTML = nav(idocmd,item);
        ...
        // replace
        var phReg = new RegExp('^'+todir + 'md\/','g')
        item = item.replace(phReg,'');



        // 指定HTML路径
        _path = item === 'index.md' ? 
            (process.cwd() + '/' + item).replace('.md',".html"):
