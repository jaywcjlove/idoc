...
function runTask(commander){
    if(commander.Create) file.mkdirsSync(commander.Create);
    //建package.json文件
    inquirer.prompt([{
        ...
        if(commander.init){
            ...
            var files,mdArr=[],mdurl = process.cwd() + '/md';

            // 判断md 目录存在不生成目录
            if( !file.exists(mdurl) ) fs.mkdirSync(mdurl,0777);

            if(commander.args.length>0){
                // 判断是否指定某个目录里面 md 拷贝到当前项目目录下
                commander.args.forEach(function(item,idx){
                    copy(item,answers.idoc.todir+'md/',{
            ...
            _.filter(file.currentFile(process.cwd()+'/'),function(_filename){
            ...
            // 读取 md 下面所有的md文件
            files = fs.readdirSync(process.cwd() + '/md/');

            // 循环找出 md 文件
            files.forEach(function(itm,idx){
                if(itm.match(/\.(md)$/i)) mdArr.push(itm);
            })
            if(mdArr.length <= 0){
