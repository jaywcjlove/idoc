...
export const rehypeAutolinkHeadings = async () => {
  ...
  const autolink = await (await (Function('return import("rehype-autolink-headings")')()) as Promise<typeof import("rehype-autolink-headings")>);
...
export const rehypeIgnore = async () => {
  // https://github.com/microsoft/TypeScript/issues/43329#issuecomment-922544562
  const ignore = await (await (Function('return import("rehype-ignore")')()) as Promise<typeof import("rehype-ignore")>);
  return ignore.default;
}

export const markdownToHTML = async (str: string, opt: Options) => {
  // https://github.com/microsoft/TypeScript/issues/43329#issuecomment-922544562
  const markdown = await (await (Function('return import("@wcj/markdown-to-html")')()) as Promise<typeof import("@wcj/markdown-to-html")>);
