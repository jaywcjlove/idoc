...
function writeFile(path,str){
    ...
    fs.writeFile(path,str,function(err,info){
        ...
        var themeurl = template + '/theme/' + default_package.idoctheme;
        ...
        //         if(_file.match(/\.(json)$/i) !== null) {
        //             console.log("_file1:",_file);
        //             return true;
        //         }else{
        //             return false;
        //         }
        ...
        //复制生成 css 的所有文件
        ncp(themeurl+'/source/css/', todir+'css', {filter:function(_file){
            if(_file.match(/\.(css)$/i) === null) {
                console.log("_file:",_file.match(/\.(css)$/i) === null,_file);
                return true;
            }else{
                return false;
            }
        ...
        //复制 img 里面的图片复制到 img 里面的
        ncp(themeurl+'/source/img', todir+'img', function (err) {
            if(err) return console.error(err);
        });
