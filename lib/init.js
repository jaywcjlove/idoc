...
function writeFile(path,str){
    ...
    fs.writeFile(path,str,function(err,info){
        ...
        copyDir(__dirname, process.cwd(),function(_stat, _path, _file){
            var stat = false;
        ...
        copyDir(themeurl, process.cwd());
        console.log(color.green('Initialization is successful! Please run "idoc build" command!'));
    })
}
...
exports.init = function(commander){
    if(fs.existsSync(template + 'idoc-theme/default/')){
        ...
        console.log(color.red("Initialization failed!"));
    }
};