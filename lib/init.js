...
function runTask(commander){
    ...
    inquirer.prompt([{
        ...
        //需要生成的md文件路径
        if(commander.init){
            var files,mdArr=[],mdurl = path.normalize(todir + '/md');
            // 判断md 目录存在不生成目录
            ...
            //递归获取 md 下面所有的md文件
            mdArr = file.readMDJSONSync(todir + 'md/');

            //获取当前目录下的所有 md 文件
            files = file.currentFile(todir + 'md/').map(function(elem, index) {
            ...
            if(mdArr.length <= 0 || files.length === 0){
                file.write( todir + 'md/index.md','# '+answers.name);
                //添加到数组的第一个位置
